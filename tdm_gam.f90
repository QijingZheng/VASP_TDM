...
!  end subroutine
  ...
  subroutine TDMAB(bra, ket, MySys, tdipole, ovlap)
    ...
    real(kind=q) :: G1, G2, G3, K1, K2, K3, vtmp(3)
    real(kind=q) :: dE
    complex(kind=q), parameter :: JJ = (0.0, 1.0_q)
    ! real(kind=q) :: norm
    ! real(kind=q) :: vsum(3)
    integer :: maxNplws, minNplws

    if (bra%ikpts /= ket%ikpts) then
      write(*,*) "Error! K vector of BRA and KET should be the same!"
      stop
      ! maxNplws = max(MySys%NPLWS(bra%ikpts), MySys%NPLWS(ket%ikpts))
      ! minNplws = min(MySys%NPLWS(bra%ikpts), MySys%NPLWS(ket%ikpts))
    end if
